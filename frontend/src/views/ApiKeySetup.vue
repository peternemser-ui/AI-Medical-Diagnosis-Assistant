    ...
    <div class="relative z-10 max-w-md w-full space-y-6">
      ...
      <div class="bg-slate-900/80 backdrop-blur-xl rounded-2xl border border-slate-800 shadow-2xl overflow-hidden">
        ...
        <div class="p-6 space-y-5">
          ...
          <p class="text-[10px] text-slate-600 text-center">Skip uses basic mode without AI agents. You can add a key later in Settings.</p>
        </div>
      </div>
...
export default {
  ...
  setup() {
      ...
      try {
        ...
        try {
          ...
          if (vendorResult && vendorResult.valid) {
            ...
            // Backend might not support validation yet — proceed anyway
            success.value = 'Key saved! Redirecting...'
            setTimeout(() => router.push('/consult'), 800)
          }
        } catch (validationErr) {
          // Validation endpoint not available — save and proceed
    ...
    return { provider, apiKey, showApiKey, showHelp, isLoading, error, success, agents, saveAndContinue, skipSetup }
  }
}
</script>
