  ...
  <div class="min-h-screen flex" :class="isDark ? 'bg-slate-950 text-white' : 'bg-slate-50 text-slate-900'">
    ...
    >
      ...
      <nav class="flex-1 py-3 px-2 overflow-y-auto">
        <template v-for="(group, gi) in navGroups" :key="group.section">
          <div
            v-if="!sidebarCollapsed"
            class="text-[9px] uppercase tracking-widest font-bold px-3"
            :class="[gi > 0 ? 'mt-5' : 'mt-1', 'mb-2', isDark ? 'text-slate-600' : 'text-slate-400']"
          >{{ group.section }}</div>
          <div v-else-if="gi > 0" class="mt-4"></div>
          <router-link
            v-for="item in group.items"
            :key="item.path"
            :to="item.path"
            class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all group mb-0.5"
            :class="[
              isActive(item.path)
...
// Icon definitions for nav items
const icons = {
  ...
  reviews: '<svg fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="M9 12.75L11.25 15 15 9.75M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>',
  logs: '<svg fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="M3.75 12h16.5m-16.5 3.75h16.5M3.75 19.5h16.5M5.625 4.5h12.75a1.875 1.875 0 010 3.75H5.625a1.875 1.875 0 010-3.75z"/></svg>',
...
const navGroups = [
  {
    section: 'Operations',
    items: [
      { path: '/admin', label: 'Overview', icon: icons.overview },
      { path: '/admin/agents', label: 'Agents', icon: icons.agents },
      { path: '/admin/cases', label: 'Cases', icon: icons.cases },
      { path: '/admin/reviews', label: 'Reviews', icon: icons.reviews },
      { path: '/admin/logs', label: 'Logs', icon: icons.logs },
    ]
  },
  {
    ...
    items: [
      ...
      { path: '/admin/models', label: 'Models', icon: icons.models },
      { path: '/admin/improve', label: 'Improve', icon: icons.improve },
      { path: '/admin/system', label: 'System', icon: icons.system },
    ]
  },
  {
    section: 'Growth',
    items: [
      { path: '/admin/billing', label: 'Billing', icon: icons.billing },
      { path: '/admin/seo', label: 'SEO', icon: icons.seo },
      { path: '/admin/referrals', label: 'Referrals', icon: icons.referrals },
      { path: '/admin/developer', label: 'Developer API', icon: icons.developer },
    ]
  },
  {
    section: 'Platform',
