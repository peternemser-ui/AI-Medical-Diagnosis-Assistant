<template>
  <div class="space-y-6">
    <APageHeader title="Improvement Dashboard" subtitle="Run quality audits and track system improvement opportunities" :showRefresh="false">
      <template #actions>
        <button
          @click="runAudit"
          :disabled="loading"
    ...
    <div v-if="!audit && !loading" class="rounded-2xl border"
      :class="isDark ? 'bg-slate-900/80 border-slate-800' : 'bg-white border-slate-200 shadow-sm'">
      <AEmptyState
        title="No audit data yet"
        description="No audit data yet. Click 'Run Quality Audit' to analyze recent cases and generate improvement recommendations."
        actionLabel="Run Quality Audit"
        @action="runAudit"
      />
    </div>
