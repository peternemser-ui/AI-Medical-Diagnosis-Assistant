  ...
  <nav role="navigation" aria-label="Main navigation"
    ...
    <div class="flex items-center gap-3">
      <router-link to="/" class="flex items-center gap-2.5 group">
        ...
        <span class="text-base font-semibold hidden sm:inline transition-colors" :class="isDark ? 'text-white group-hover:text-indigo-300' : 'text-slate-900 group-hover:text-indigo-600'">{{ t('nav.brand') }}</span>
      </router-link>

      <!-- Home link -->
      <router-link to="/" class="hidden sm:flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-sm font-medium transition-all relative" :class="currentPage === 'home' ? (isDark ? 'text-indigo-400 font-semibold' : 'text-indigo-700 font-semibold') : (isDark ? 'text-slate-300 hover:text-white hover:bg-slate-700/60' : 'text-slate-600 hover:text-slate-900 hover:bg-slate-100')" :aria-current="currentPage === 'home' ? 'page' : undefined">
        ...
        {{ t('nav.home') }}
        <span v-if="currentPage === 'home'" class="absolute bottom-0 left-1/2 -translate-x-1/2 w-4 h-0.5 rounded-full bg-indigo-600"></span>
      </router-link>
    ...
    <div class="hidden sm:flex items-center gap-1">
      <!-- Consult -->
      <router-link to="/consult" class="flex items-center gap-1.5 px-2.5 py-2 rounded-lg text-sm font-medium transition-all relative"
        :aria-current="currentPage === 'consult' ? 'page' : undefined"
        :class="[
          currentPage === 'consult'
            ? (isDark ? 'text-indigo-400 font-semibold bg-indigo-500/10' : 'text-indigo-700 font-semibold bg-indigo-50')
        ...
        <span v-if="currentPage === 'consult'" class="absolute bottom-0 left-1/2 -translate-x-1/2 w-4 h-0.5 rounded-full bg-indigo-600"></span>
      </router-link>

      <!-- Reports -->
      <router-link to="/reports" class="flex items-center gap-1.5 px-2.5 py-2 rounded-lg text-sm font-medium transition-all relative"
        ...
        <span v-if="currentPage === 'reports'" class="absolute bottom-0 left-1/2 -translate-x-1/2 w-4 h-0.5 rounded-full bg-indigo-600"></span>
      </router-link>

      <!-- Medications -->
      <router-link to="/medications" class="flex items-center gap-1.5 px-2.5 py-2 rounded-lg text-sm font-medium transition-all relative"
        ...
        <span v-if="currentPage === 'medications'" class="absolute bottom-0 left-1/2 -translate-x-1/2 w-4 h-0.5 rounded-full bg-indigo-600"></span>
      </router-link>
      ...
      <div class="relative" ref="userMenuRef">
        <button v-if="isLoggedIn" @click="showUserMenu = !showUserMenu" aria-label="User menu" :aria-expanded="showUserMenu" class="flex items-center gap-1.5 px-2 py-1.5 rounded-lg transition-all" :class="isDark ? 'hover:bg-slate-700/60 text-slate-400 hover:text-white' : 'hover:bg-slate-100 text-slate-500 hover:text-slate-900'">
          <div class="w-7 h-7 rounded-full bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center text-detail font-bold text-white">
            {{ userInitials }}
          </div>
        ...
        <Transition enter-active-class="transition duration-150 ease-out" enter-from-class="opacity-0 scale-95" enter-to-class="opacity-100 scale-100" leave-active-class="transition duration-100 ease-in" leave-from-class="opacity-100 scale-100" leave-to-class="opacity-0 scale-95">
          <div v-if="showUserMenu && isLoggedIn" class="absolute right-0 top-full mt-2 w-52 rounded-xl shadow-xl border z-50 overflow-hidden" :class="isDark ? 'bg-slate-900 border-slate-700/60' : 'bg-white border-slate-200'">
            <!-- User info header -->
            <div class="px-4 py-3 border-b" :class="isDark ? 'border-slate-800' : 'border-slate-100'">
              <div class="flex items-center gap-3">
                ...
                <div class="min-w-0">
                  ...
                  <div class="text-xs truncate" :class="isDark ? 'text-slate-500' : 'text-slate-400'">{{ userEmail || 'No email set' }}</div>
                </div>
              </div>
            </div>
            <!-- Menu items -->
            <div class="py-1">
              <router-link to="/profile" @click="showUserMenu = false" class="flex items-center gap-2.5 px-4 py-2.5 text-sm transition-colors" :class="isDark ? 'text-slate-300 hover:bg-slate-800 hover:text-white' : 'text-slate-600 hover:bg-slate-50 hover:text-slate-900'">
              ...
              <router-link to="/settings" @click="showUserMenu = false" class="flex items-center gap-2.5 px-4 py-2.5 text-sm transition-colors" :class="isDark ? 'text-slate-300 hover:bg-slate-800 hover:text-white' : 'text-slate-600 hover:bg-slate-50 hover:text-slate-900'">
              ...
              <router-link to="/reports" @click="showUserMenu = false" class="flex items-center gap-2.5 px-4 py-2.5 text-sm transition-colors" :class="isDark ? 'text-slate-300 hover:bg-slate-800 hover:text-white' : 'text-slate-600 hover:bg-slate-50 hover:text-slate-900'">
                <svg class="w-4 h-4 opacity-60" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 17v-2m3 2v-4m3 4v-6m2 10H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
                {{ t('nav.reports') }}
              </router-link>
              <router-link to="/setup" @click="showUserMenu = false" class="flex items-center gap-2.5 px-4 py-2.5 text-sm transition-colors" :class="isDark ? 'text-slate-300 hover:bg-slate-800 hover:text-white' : 'text-slate-600 hover:bg-slate-50 hover:text-slate-900'">
                <svg class="w-4 h-4 opacity-60" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 7a2 2 0 012 2m4 0a6 6 0 01-7.743 5.743L11 17H9v2H7v2H4a1 1 0 01-1-1v-2.586a1 1 0 01.293-.707l5.964-5.964A6 6 0 1121 9z"/></svg>
                {{ t('nav.apiKeys') }}
              </router-link>
              <router-link v-if="userProfile?.role === 'admin'" to="/admin" @click="showUserMenu = false" class="flex items-center gap-2.5 px-4 py-2.5 text-sm transition-colors" :class="isDark ? 'text-violet-400 hover:bg-slate-800' : 'text-violet-600 hover:bg-violet-50'">
                <svg class="w-4 h-4 opacity-80" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 3v2m6-2v2M9 19v2m6-2v2M5 9H3m2 6H3m18-6h-2m2 6h-2M7 19h10a2 2 0 002-2V7a2 2 0 00-2-2H7a2 2 0 00-2 2v10a2 2 0 002 2zM9 9h6v6H9V9z"/></svg>
                Admin Dashboard
              </router-link>
              <router-link to="/help" @click="showUserMenu = false" class="flex items-center gap-2.5 px-4 py-2.5 text-sm transition-colors" :class="isDark ? 'text-slate-300 hover:bg-slate-800 hover:text-white' : 'text-slate-600 hover:bg-slate-50 hover:text-slate-900'">
            ...
            <div class="border-t" :class="isDark ? 'border-slate-800' : 'border-slate-100'"></div>
            <div class="py-1">
              <button @click="handleLogout" class="flex items-center gap-2.5 px-4 py-2.5 text-sm transition-colors w-full text-left" :class="isDark ? 'text-red-400 hover:bg-slate-800' : 'text-red-500 hover:bg-red-50'">
    ...
    <div class="flex sm:hidden items-center gap-1">
      <button @click="showMobileMenu = !showMobileMenu" class="p-2 rounded-lg transition-colors" :class="isDark ? 'text-slate-400 hover:bg-slate-800' : 'text-slate-500 hover:bg-slate-100'" aria-label="Toggle navigation menu" :aria-expanded="showMobileMenu">
        <svg v-if="!showMobileMenu" class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h16M4 18h16"/></svg>
    ...
    >
      <div v-if="showMobileMenu" class="absolute top-full left-0 right-0 border-b shadow-xl sm:hidden" style="z-index: 9999"
        :class="isDark ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200'">
        <!-- Primary navigation -->
        <div class="px-3 pt-3 pb-1">
          <p class="px-3 mb-1 text-xs font-semibold uppercase tracking-wider" :class="isDark ? 'text-slate-600' : 'text-slate-400'">Navigation</p>
          <router-link to="/consult" @click="showMobileMenu = false"
            class="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium"
            :class="currentPage === 'consult' ? (isDark ? 'bg-indigo-500/10 text-indigo-400' : 'bg-indigo-50 text-indigo-700') : (isDark ? 'text-slate-300 hover:bg-slate-800' : 'text-slate-700 hover:bg-slate-50')">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z"/></svg>
            {{ t('nav.newConsultation') }}
          </router-link>
          <router-link to="/reports" @click="showMobileMenu = false"
            class="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium"
            :class="currentPage === 'reports' ? (isDark ? 'bg-indigo-500/10 text-indigo-400' : 'bg-indigo-50 text-indigo-700') : (isDark ? 'text-slate-300 hover:bg-slate-800' : 'text-slate-700 hover:bg-slate-50')">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M9 17v-2m3 2v-4m3 4v-6m2 10H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
            {{ t('nav.reports') }}
          </router-link>
          <router-link to="/medications" @click="showMobileMenu = false"
            class="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium"
            :class="currentPage === 'medications' ? (isDark ? 'bg-indigo-500/10 text-indigo-400' : 'bg-indigo-50 text-indigo-700') : (isDark ? 'text-slate-300 hover:bg-slate-800' : 'text-slate-700 hover:bg-slate-50')">
        ...
        <!-- Explore section -->
        <div class="px-3 py-2 border-t" :class="isDark ? 'border-slate-800' : 'border-slate-100'">
          <p class="px-3 mb-1 text-xs font-semibold uppercase tracking-wider" :class="isDark ? 'text-slate-600' : 'text-slate-400'">Explore</p>
        ...
        <div class="px-3 py-2 border-t" :class="isDark ? 'border-slate-800' : 'border-slate-100'">
          <template v-if="isLoggedIn">
            <!-- User profile row -->
            <div class="flex items-center gap-3 px-3 py-3 mb-1 rounded-xl" :class="isDark ? 'bg-slate-800/60' : 'bg-slate-50'">
              <div class="w-9 h-9 rounded-full bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center text-sm font-bold text-white flex-shrink-0">{{ userInitials }}</div>
              <div class="min-w-0">
                <div class="text-sm font-semibold truncate" :class="isDark ? 'text-white' : 'text-slate-900'">{{ userName }}</div>
                <div class="text-xs truncate" :class="isDark ? 'text-slate-500' : 'text-slate-400'">{{ userEmail || '' }}</div>
              </div>
            </div>
            <router-link to="/profile" @click="showMobileMenu = false" class="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm" :class="isDark ? 'text-slate-300 hover:bg-slate-800' : 'text-slate-700 hover:bg-slate-50'">
              <svg class="w-4 h-4 opacity-60" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M5.121 17.804A13.937 13.937 0 0112 16c2.5 0 4.847.655 6.879 1.804M15 10a3 3 0 11-6 0 3 3 0 016 0zm6 2a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
              {{ t('nav.profile') }}
            </router-link>
            <router-link to="/settings" @click="showMobileMenu = false" class="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm" :class="isDark ? 'text-slate-300 hover:bg-slate-800' : 'text-slate-700 hover:bg-slate-50'">
          ...
          <router-link v-else to="/profile" @click="showMobileMenu = false" class="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium" :class="isDark ? 'text-indigo-400 hover:bg-slate-800' : 'text-indigo-700 hover:bg-indigo-50'">
          ...
          <div class="px-3 py-2 mt-1 border-t" :class="isDark ? 'border-slate-800' : 'border-slate-100'">
            <ThemeLangControls />
          </div>
        </div>
      </div>
